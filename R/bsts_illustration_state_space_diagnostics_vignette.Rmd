...
Start by loading the necessary R libraries. 
...
st.sp.lists <- list(
  ...
  `4a_ar` = c('AddLocalLevel','AddAr','AddSeasonal','AddRegression'),
  `4b_trend` = c('AddSemilocalLinearTrend','AddSeasonal','AddRegression')#,
)
## MAIN SIM SETTINGS
...
bsts.niters <- list(sm.start = 200, sm.max = 400, lg.start = 1000, lg.max = 4000)
```
...
simlist[keyid:keyid] <- fitBstsUpdateSimlist(simlist[keyid:keyid],  ## temp list of only 1 item name==key
                               ...
                               bsts.niter = bsts.niters$sm.start, ## **START at MAX niter for large npds **
...
## 2. Check MCMC Convergence and Diagnostics 
...
convergence is not yet met for a given number of iterations. Setting
...
posterior predictive distributions. So when we recheck convergence and fit diagnostics, 
these should generally show improvement (or at least parity) with the model fit from 
fewer MCMC iterations. 
...
## 3. Add Regression Component and Compare BSTS State Spaces

Next we add a seasonality to the state space ("seasonal.52.1" = one cycle of 52
periods per year). This is evident in the seasonal (yearly) cycles it adds to 
state components. The augmented state space then improves model performance in 
predicting during the pre-intervention period. 
...
simlist[keyid:keyid] <- fitBstsUpdateSimlist(simlist[keyid:keyid],  ## temp list of only 1 item name==key
                               ...
                               plot.show = F, plot.save=F, verbose=F) 
##
bsts.model <- simlist[[key]]$compare$bsts$quadratic[[1]]$CausalImpact$model$bsts.model
plotBstsStateComps(bsts.model, intpd)
```

Adding a regression component to the state space largely improves model
fit, evidenced by the substantial reducing in mean aboluste error (MAE), 
...
model before the intervention, which is then used for forecasting
post-intervention (i.e., the counterfactual). Here the plot of expected 
inclusion probabilities indicates the likelihood that each covariate is used 
in the model (i.e., used to create the synthetic control group) over the 
iterations in the MCMC draws sampling from the posterior predictive distribution
of the outcome (Y).
...
```{r bsts_compare}
m1 <- simlist$`1_level`$compare$bsts$quadratic[[1]]$CausalImpact$model$bsts.model
m2 <- simlist$`2_season`$compare$bsts$quadratic[[1]]$CausalImpact$model$bsts.model
m3 <- simlist$`3_regression`$compare$bsts$quadratic[[1]]$CausalImpact$model$bsts.model
...
added to the local level (trend without slope). This helps reduce error mostly 
in the early periods before the model can 'learn' a good fit. This is evident
in the vertical gaps between the observed (black dots) and predicted (blue line) 
values decreasing over time until the deviation nearly vanishes by the end.  
However, while the autoregressive component (AR[1]) improves model fit
(i.e., decrased MAE), it also runs the risk of overfitting -- as evidenced by 
the fact that no autoregressive component was included in the data generating 
process.
