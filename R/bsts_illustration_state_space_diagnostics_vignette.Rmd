...
predicting close to the observed values (top-center panel).
```{r mcmc_diag_pass, fig.dim = c(14, 10), out.width="100%"}
convcheck <- postPredChecks(causimp, save.plot=F, return.val = T)
```
To account for seasonality in the time series, we add a seasonal component 
to the state space ("seasonal.52.1" = one cycle of 52 periods per year). 
