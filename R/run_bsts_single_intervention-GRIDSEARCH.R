  ...
  for (i in 1:length(simlist))
  {
    ...
    for (h in 1:length(bsts.state.specs)) 
    {
      ...
      for (k in 1:length(effect.types)) 
      {
        ...
        simlist[[key]]$compare$bsts[[effect.type]]$CausalImpact <- impact_amount
        simlist[[key]]$compare$bsts[[effect.type]]$cumu.pred.error <-  cumsum(colSums(abs(bsts.pred.er)))
...
# ##_-------------------------------
# ## Save Main Sim Grid Search Run
# ##_-------------------------------
# sim.id <- round(10*as.numeric(Sys.time()))
simlist <- runSimBstsDiDComparison(simlist, plot.show = T, plot.save = T)
