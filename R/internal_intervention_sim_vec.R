...
## 
##=================================
runSimInternalIntervention <- function(
...
###
runSimInternalInterventionEffectComparison <- function(
    effect.types,
    ## Simulation base settings
    n = 150, ## NUMBER OF actors  
    npds = 80, ## NUMBER OF PERIODS
    ystart = 0,
    effect.type = 'constant',  ## c('constant','quadratic','geometric'), ## treatment effect shapes
    treat.rule = 'below.benchmark',
  ...
  for (k in 1:length(effect.types)) {
    ...
    out <- runSimInternalIntervention(
      n, npds, ystart, 
      effect.type, treat.rule, treat.threshold, 
