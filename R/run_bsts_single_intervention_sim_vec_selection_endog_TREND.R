...
for (i in 1:length(simlist))
{
  ...
  for (k in 1:length(effect.types)) 
  {
    ...
    predictors <- as.matrix(dat[, ! names(dat) %in% 'treatment_y_mean']) ## remove response; convert to matrix
    
    ## State Space
    st.sp <- list()
    # st.sp <- AddStaticIntercept(st.sp, y.pre.treat.NAs.post.treat)
    # st.sp <- AddLocalLevel(st.sp, y.pre.treat.NAs.post.treat)
    st.sp <- AddAr(st.sp, y.pre.treat.NAs.post.treat, lags = 1)
    # st.sp <- AddAutoAr(st.sp, y.pre.treat.NAs.post.treat)
    # st.sp <- AddLocalLinearTrend(st.sp, y.pre.treat.NAs.post.treat)
    # st.sp <- AddLocalLinearTrend(list(), y.pre.treat.NAs.post.treat)
    st.sp <- AddGeneralizedLocalLinearTrend(st.sp, y.pre.treat.NAs.post.treat, )
    # st.sp <- AddDynamicRegression(st.sp, y.pre.treat.NAs.post.treat ~ predictors)
    ...
    png(filename = sprintf('single_intervention_BSTS_dynamic_treatment_effect_comparison_%s_%s_%s.png',
                           key,effect.type,sim.id))
