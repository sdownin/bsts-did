---
title: "Towards Dynamic Causal Inference: A Bayesian Counterfactual Approach"
output: pdf_document
---

This is a tutorial and illustration for (1) running Bayesian structural time series 
(BSTS) models, including configuring the state space and checking model diagnostics.
The fitted BSTS model is then used for (2) counterfactual causal inference. 
Additionally, the last two sections present simulation results for exploring
(3) BSTS causal inference on different causal cycle curve shapes,
and (4) sensitivity to Bayesian priors. These include regression spike-and-slab
prior attributes, such as expected model size and expected r^2^; and finally, 
the quality of the covariate series (in terms of the stability of the 
association with the outcome series for modeling the counterfactual).  
...
npds <- 520
intpd <- round( npds * .6 )
effect.type <- 'constant' ##  c('constant','geometric','quadratic') ## c('quadratic')  
...
# 1. Bayesian Structural Time Series (BSTS) Overview
...
st.sp3 <- AddSeasonal(st.sp3, y.pre.treat.NAs.post, 
  ...
  sigma.prior = SdPrior(
    ...
    upper.limit= 1.5 * y.sd  ##try a factor of empirical y SD to set upper limit
  ),
  initial.state.prior = NormalPrior(
...
```
This appears to have a worse fit, in terms of slightly increased MAE,
but the model now have seasonal structure that would inform a better
counterfactual post-intervention. So, essentially, the addition of seaonsal
structure reduces a portion of the overfitting against current data
in order to incorporate a structure (seasonality) that can continue into the
post-intervention forecast that will be used as the counterfactual 
for causal inference. 

In addition to seasonality, another way to incorporate structure from into
the training window that will continue into the post-intervention window
...
about the outcome series pre-intervention (and assuming that relationship
...
covariates so the regression contribution to the state space in the 
pre-intervention (training) window is added to the state sapce structure
that carried forward post-intervetion -- and therefore improves the 
representativeness of the counterfactual series.
...
st.sp4 <- AddSeasonal(st.sp4, y.pre.treat.NAs.post, 
  ...
  season.duration = 1, 
  sigma.prior = SdPrior(
    sigma.guess = 0.01 * y.sd,      ## use empirical y SD
    sample.size = round(0.1 * (intpd-1)),  ## use half of pre-int window
...
the outcomes series in the pre-intervention window. In Bayesian structural time 
series models, variable selection is incorporated into the model within a 
Bayesian framework via spike-and-slab priors. 
...
bsts.fit5 <- bsts(y.pre.treat.NAs.post ~ .,
                 ...
                 data = predictors,
                 expected.model.size = 9,  ## argument passed to SpikeSlabPrior
                 niter = bsts.niter,
                 seed = rand.seed, ping=0)
...
summary(bsts.fit5)
par(mfrow=c(3,1))
plot(bsts.fit5, 'coefficients', main='Expected Size = 9')
plot(bsts.fit4, 'coefficients', main='Expected Size = 3')
plot(bsts.fit51, 'coefficients', main='Expected Size = 1')
CompareBstsModels(list(`Expect Size=9`=bsts.fit5,
                       `Expect Size=3`=bsts.fit4,
                       `Expect Size=1`=bsts.fit51))
```
...
## 1.6. Compare State Space Trends 
...
### 1.6.0. Local Level (Trend without Slope)
...
### 1.6.1. Local linear trend (Trend with Level and Slope)
...
### 1.6.2. Semilocal linear trend (Trend with Level and slope with AR(1) drift)
...
the seasonality structure into the forecast (via the 
Kalman filter recursive least squares). 

```{r, out.width="100%", fig.align="center", fig.dim=c(12, 7)}
par(mfrow=c(1,2))
plotBstsStateComps(bsts.fit9, pd.ids=1:104) ## 2 cycles (of 52 season pds)
plotBstsStateComps(bsts.fit9, pd.ids=1:520) ## 10 cycles (of 52 season pds)
```
...
```{r, out.width="100%", fig.align="center", fig.dim=c(14, 10)}  
###
...
convergence of the MCMC chains. Similarly, there is need to assess how the 
...
# 3. Dynamic Causal Inference: MCC Curve Shapes

```{r} 
## Quadratic (inverted U-shape) shape (slow onset, slow decay)
datqu <-  processSimData(simlist, key, 
                           effect.type = 'quadratic', 
                           dgp.prior.sd.weight=0.01, 
                           sim.id=sim.id)
## Geometric decay shape (fast onset, fast decay)
datge <-  processSimData(simlist, key, 
                           effect.type = 'geometric', 
                           dgp.prior.sd.weight=0.01, 
                           sim.id=sim.id)

# datqu$bsts.df1.filepath

## LOAD FROM FILE
dfqu <- read.csv(datqu$bsts.df1.filepath, stringsAsFactors = F) ## don't auto convert chars to factor type
dfqu <- as_tibble(dfqu) ## wrap in a class with extra capabilities
print(dfqu)
##
dfge <- read.csv(datge$bsts.df1.filepath, stringsAsFactors = F) ## don't auto convert chars to factor type
dfge <- as_tibble(dfge) ## wrap in a class with extra capabilities
print(dfge)
```

```{r}

## FUNCTION TO MAKE THE BSTS MODEL AND FIT IT FOR A GIVEN DATAFRAME
applyChosenBstsModelToSimData <- function(dfx, intpd, plots.show=TRUE) 
{
  ##------------ DATA SET -------------------------
  npds <- nrow(dfx)
  ## INDICES OF PRE-INTERVENTION and POST-INTERVENTION WINDOWS
  ...
  post.period.response <- y[post.idx]
  ## Covariates (predictors) - data for the "formula = y ~ predictors" argument
  predictors <- dfx[ , ! names(dfx) %in% outcome.col]
  ## Eppirical y SD for scaling prior settings
  y.sd <- sd(y[pre.idx], na.rm=T)
  ...
  # ##----- REGRESSSION PRIOR: SPIKE & SLAB -------------------
  # ##**MOVED TO bsts() function expected.model.size argument**
  # prior.spikes <- c(.7,.8,.9,.1,.1,.1,.1,.1,.1,.1) ## +1 column for intercept (?)
  # prior.means <- c(.2,.2,.3,0,0,0,0,0,0,0) ## +1 column for intercept (?)
  # # Directly set prior spike probabilities for each covariate in the prior object
  # priorObjB <- SpikeSlabPrior(x = model.matrix(y.pre.treat.NAs.post ~ ., 
...
Now run the function to create and fit the BSTS models on each data set
and then run `CausalImpact()` to compute the causal inference results
for comparison.  

```{r}
bsts.fit9.qu <- applyChosenBstsModelToSimData(dfqu, intpd, plots.show = T) 
impact9.qu <- CausalImpact(bsts.model = bsts.fit9.qu,
                              post.period.response = post.period.response,
                              alpha=0.05, model.args=list(niter = bsts.niter)) 
```
...
```{r}
bsts.fit9.ge <- applyChosenBstsModelToSimData(dfge, intpd, plots.show = T) 
impact9.ge <- CausalImpact(bsts.model = bsts.fit9.ge,
                              post.period.response = post.period.response,
                              alpha=0.05, model.args=list(niter = bsts.niter)) 
```
```{r}
...
## 4.1. BSTS Prior SD Settings

```{r}
##
##**TODO**
##**CHECK IF THIS CAN BE DROPPED - BRIEFLY ADDRESSED IN 1.4**
##
```


## 4.2. Regression Features

### 4.2.1. Model Size (model selects how many candidate series in counterfactual)

```{r}
## checking default, others (1,4,7)
##
##**TODO THIS BEFORE 4.1**
```


### 4.2.2.  Expected R^2^ (how much outcome does the model explain)
...
### 4.2.3. ##???### Candidate Predictors (series from which the model selects)
...
```{r,  out.width="100%", fig.align="center", fig.dim=c(7, 12), message=FALSE, warning=FALSE}
## Show predictor series and some measure, like correlation
sim.reps.id <- 16745322384
dfr <- read.csv( file=file.path(dir_rep, sprintf('bsts_runs_%s.csv',sim.reps.id)), 
                stringsAsFactors = F)

# ggplot(dfr, aes(x=err.pre.me, fill=factor(cov.has.control), 
#                colour=factor(cov.has.control))) +  
#   xlim(c(-.1,.1)) +
#   geom_density(alpha=.15) +
#   geom_vline(xintercept=0,lty=3) + theme_bw()
# ggplot(dfr, aes(x=err.pre.me, fill=factor(cov.has.control), colour=factor(cov.has.control))) +
#   xlim(c(-.1,.1)) +
#   geom_histogram(alpha=.15, binwidth = 0.01, position = 'identity') +
#   geom_vline(xintercept=0,lty=3) + theme_bw()
# ###
# ggplot(dfr, aes(x=bias.post.me, fill=factor(cov.has.control), colour=factor(cov.has.control))) +
#   xlim(c(-1,1)) +
#   geom_density(alpha=.15) +
#   geom_vline(xintercept = 0, lty=3) + theme_bw()
# ggplot(dfr, aes(x=bias.post.me, fill=factor(cov.has.control), colour=factor(cov.has.control))) +
...
df.bsts.did <- rbind(
  data.frame(model='bsts',
             err.post.me=dfr$bias.post.me[dfr$cov.has.control==1],
             cov.has.ctrl=1, cov.type=cov.type.ctrl, effect.type=dfr$effect.type),
  data.frame(model='bsts',
             err.post.me=dfr$bias.post.me[dfr$cov.has.control==0],
             cov.has.ctrl=0, cov.type=cov.type.no, effect.type=dfr$effect.type),
  data.frame(model='did',
             err.post.me=dfr$did.post.me[dfr$cov.has.control==1],
             cov.has.ctrl=1, cov.type=cov.type.ctrl, effect.type=dfr$effect.type),
  data.frame(model='did',
             err.post.me=NA,
             cov.has.ctrl=0, cov.type=cov.type.no, effect.type=dfr$effect.type)
