...
# ## OR USE CACHED DATA OBJECT IF JUST CREATED
# df1 <- dat1$bsts.df1
df1 <- as_tibble(df1)
print(df1)
```
...
```{r}
# y.pre <- df1$y_treatment
# post.ids.NA <- (1:npds) > intpd
# y.pre[post.ids.NA] <- NA
...
predictors <- df1[, ! names(df1) %in% 'y_treatment'] ## remove response; convert to matrix
...
The local linear trend is useful for short term predictions of se
```{r}
## Initiate empty state space configuration list
st.sp3 <- list()
## Add local level to trend component of state space
...
Useful for longer-term forecasts (linear trend persists longer, 
tighter prediction bounds over longer post-intervention window)
```{r}
## Initiate empty state space configuration list
st.sp4 <- list()
## Add local level to trend component of state space
...
## Including a Control Group Series (in addition to other covariate series)
...
convcheck <- bstsPostPredChecks(causimp, save.plot=F, return.val = T)
```
