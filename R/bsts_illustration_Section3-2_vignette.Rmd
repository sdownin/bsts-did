...
data.filepath <- file.path(dir_proj, '__bsts_vignette_illustra_sim_df.csv')
if ( ! file.exists(data.filepath) ) {
  cat(sprintf('Illustration data file 1 not found in project dir:\n %s.\n Simulating new illustration data set.\n',dir_proj))
...
bsts.df1.filepath <- file.path(dir_proj, '__bsts_vignette_illustra_sim_agg_bsts_df1-NOctrl.csv')
bsts.df2.filepath <- file.path(dir_proj, '__bsts_vignette_illustra_sim_agg_bsts_df2-ONEctrl.csv')
...
if ( file.exists(data.filepath) & any(!f1exists, !f2exists)) {  # !f3exists
  ...
  if (!f1exists) {
      cat(sprintf('Illustration data file 2 not found in project dir:\n %s.\n Aggregating new illustration data set.\n',dir_proj))
    cov.df.wide1 <- simdf %>%
      dplyr::filter( 
        ! is.na(match_id), 
        group=='control'
  ...
  if (!f2exists) {
      cat(sprintf('Illustration data file 3 not found in project dir:\n %s.\n Aggregating new illustration data set.\n',dir_proj))
    cov.df.wide2 <- simdf %>%
      dplyr::filter( 
        ! is.na(match_id), 
        group=='control'
...
summary(bsts.fit2)
plot(bsts.fit2)
plot(bsts.fit2, 'components')
plot(bsts.fit2, 'coefficients')
qqdist(bsts.fit2$one.step.prediction.errors)
AcfDist(bsts.fit2$one.step.prediction.errors)
checks2 <- bstsPostPredChecks(bsts.fit2, save.plot = F, return.val = T)
cat(checks2$summary)
...
summary(bsts.fit3)
plot(bsts.fit3)
plot(bsts.fit3, 'components')
plot(bsts.fit3, 'coefficients')
qqdist(bsts.fit3$one.step.prediction.errors)
AcfDist(bsts.fit3$one.step.prediction.errors)
checks3 <- bstsPostPredChecks(bsts.fit3, save.plot = F, return.val = T)
cat(checks3$summary)
```
...
summary(bsts.fit4)
plot(bsts.fit4)
plot(bsts.fit4, 'components')
plot(bsts.fit4, 'coefficients')
qqdist(bsts.fit4$one.step.prediction.errors)
AcfDist(bsts.fit4$one.step.prediction.errors)
checks4 <- bstsPostPredChecks(bsts.fit4, save.plot = F, return.val = T)
cat(checks4$summary)
```
...
```{r}
CompareBstsModels(list(level=bsts.fit, 
                       seasonal=bsts.fit2, 
