...
## 1.1. Specify a Simple State Space 
...
st.sp <- AddLocalLevel(st.sp, y.pre.treat.NAs.post, 
                       initial.state.prior = NormalPrior(mu=0, sigma = .01))
...
## 1.2. Fit BSTS Model - MCMC Estimation
...
###
getBstsDiagnosticSummary <- function(bsts.fit) {
  ...
  cat(checks$summary)
  # plot(bsts.fit, 'predictors') ## qqdist(checks$err.1step.dist) ## AcfDist(checks$err.1step.dist)
}
getBstsDiagnosticSummary(bsts.fit)
```
Then we can plot the contributions to the model's state space to help with 
visual understanding of this model and comparison with other models below.
```{r}
source(file.path(dir_r,'bsts_helper_functions.R'))
...
st.sp2 <- AddSeasonal(st.sp2, y.pre.treat.NAs.post, 
  ...
  season.duration = 1, 
  sigma.prior = SdPrior(
    sigma.guess = .01,    ## guess
    sample.size = 1,    ## default
...
getBstsDiagnosticSummary(bsts.fit2)
```
```{r}
plotBstsStateComps(bsts.fit2, intpd=intpd, filename=NA) ##NA file does not save
```
...
st.sp3 <- AddSeasonal(st.sp3, y.pre.treat.NAs.post, 
  ...
  sigma.prior = SdPrior(
    sigma.guess = 1,    ## guess
    sample.size = 1,    ## default
    upper.limit= 1.5 * sd(y.pre.treat.NAs.post, na.rm = T)
  ),
  initial.state.prior = NormalPrior(
    mu= 1,   ##guess
    sigma=1  ## guess
...
## CALL OUR CUSTOM BSTS DIAGNOSTICS SUMMARY
getBstsDiagnosticSummary(bsts.fit3)
```

## 1.5. Compare State Space Configurations 
...
include a local linear trend, or semi-local linear trend.
...
st.sp3 <- list()
## Add local level to trend component of state space
st.sp3 <- AddLocalLinearTrend(st.sp3, y.pre.treat.NAs.post, 
...
st.sp4 <- list()
## Add local level to trend component of state space
st.sp4 <- AddSemilocalLinearTrend(st.sp4, y.pre.treat.NAs.post, 
...
## CALL OUR CUSTOM BSTS DIAGNOSTICS SUMMARY
getBstsDiagnosticSummary(bsts.fit4)
```
...
## 1.6. Check MCMC Convergence and Model Diagnostics
...
## 4 Dynamic Causal Inference: Robustness to MCC Curve Shapes
...
if the well-behaving series has the same unit of analysis as the outcome, then DiD uses this as control group (counterfactual)

- bsts does not need to be parallel trend; did requires parallel trend  (refer to paper)
...
### 4.3.1. Scenario 1: Well-Behaved Control: NO

This was the default scenario introduced above. The scenarios below present
comparisons of the alignment between DGP and BSTS model. 
...
```{r}
df2 <- read.csv(dat2$bsts.df1.filepath, stringsAsFactors = F) ## don't auto convert chars to factor type
df2 <- as_tibble(df2)
print(df2)
...
## Compare BSTS models' cumulative one-step ahead prediciton error
CompareBstsModels(reslist[[scenario_key]])
```

```{r}
plotBstsStateComps(bsts.fit4, intpd=intpd, filename = NA)
```

### 4.3.2. Scenario 2: Well-Behaved Control: YES, Noise: LOW (`0.01`)

Repeat the above analyses with data simulated using a larger prior SD

```{r}
## SIMULATE DATA AND RETURN DATA LIST OBJECT
dat2 <- processSimData(simlist, key, effect.types, 
                       dgp.prior.sd.weight.high, 
                       sim.id = sim.id)
```
...
print(df2)
```
We write a function to repeat the analyses above for each subsequent 
scenario of data and model properties.
```{r}

##
##  Define a function to run all the analyses we just introduced above for
##   new data from a different scenario and including additional covariates 
##   (control series)
##
runSimulationScenarioBstsModelComparison <- function(df)
{
    ## Outcome (response) series
    y <- df$y_treatment
    ## Train on y pre-treatment but NA's post-treatment
    ...
    predictors <- df[, ! names(df) %in% 'y_treatment'] ## remove response; convert to matrix
    ...
    ## Fit BSTS model: formula “y.pre ~ .” indicates inclusion of regression  
    ## component (all covariates in df[,-1] dataframe)
    bsts.fit <- bsts(y.pre.treat.NAs.post ~ . ,
                     state.specification = st.sp,
                     data = predictors,  ## -1 excludes the outcome in 1st column 
                     niter = bsts.niter)
    ...
    qqdist(bsts.fit$one.step.prediction.errors)
    AcfDist(bsts.fit$one.step.prediction.errors)
    ...
    ##---------------------------------------------------------------------------
    ## Compare different state space trends
    ...
    st.sp3 <- AddLocalLinearTrend(st.sp3, y.pre.treat.NAs.post, 
      level.sigma.prior=SdPrior(sigma.guess=.01, sample.size=32, initial.value=.01), 
      slope.sigma.prior=SdPrior(sigma.guess=.01, sample.size=32, initial.value=.01), 
      initial.level.prior=NormalPrior(mu=0, sigma=.01, initial.value=.01), 
      initial.slope.prior=NormalPrior(mu=0, sigma=.01, initial.value=.01)
    )
    ## Add trigonometric seasonality to state space
    st.sp3 <- AddSeasonal(st.sp3, y.pre.treat.NAs.post, 
      nseasons = 52, 
...
## SD LOW SCENARIO KEY
scenario_key <- '2_noCtrl_highSd'
# 
# ## CALL THE FUNCTION TO RUN ON THE DF WITH CONTROL GROUP SERIES
# reslist[[scenario_key]] <- runSimulationScenarioBstsModelComparison(df2)
# 
# ## Compare BSTS models' cumulative one-step ahead prediciton error
# CompareBstsModels(reslist[[scenario_key]])
...
### 4.3.3. Scenario 3: Well-Behaved Control: YES, Noise: LOW (`0.01`)

```{r}
## SD LOW - WITH CONTROL GROUP
df12 <- read.csv(dat1$bsts.df2.filepath, stringsAsFactors = F)
## SD HIGH - WITH CONTROL GROUP
df22 <- read.csv(dat2$bsts.df2.filepath, stringsAsFactors = F)
```

```{r res.ctrl.sdlow}
## SD LOW SCENARIO KEY
scenario_key <- '3_hasCtrl_lowSd'
# # CALL THE FUNCTION TO RUN ON THE DF WITH CONTROL GROUP SERIES
# reslist[[scenario_key]] <- runSimulationScenarioBstsModelComparison(df12)
# ## Compare BSTS models' cumulative one-step ahead prediciton error
# CompareBstsModels(reslist[[scenario_key]])

```

### 4.3.4. Scenario 4: Well-Behaved Control: YES, Noise: HIGH (`0.1`)

```{r res.ctrl.sdhigh}
## SD LOW SCENARIO KEY
scenario_key <- '4_hasCtrl_highSd'
# # CALL THE FUNCTION TO RUN ON THE DF WITH CONTROL GROUP SERIES
# reslist[[scenario_key]] <- runSimulationScenarioBstsModelComparison(df22)
# ## Compare BSTS models' cumulative one-step ahead prediciton error
# CompareBstsModels(reslist[[scenario_key]])
```
