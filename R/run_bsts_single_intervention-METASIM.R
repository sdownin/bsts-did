...
runSimUpdateCompareBstsDiD <- function(simlist,     ## n, npds, intpd moved into simlist elements
  ...
  for (i in 1:length(simlist))
  {
    ...
    if ( ! is.na(save.items.dir) ) {
      ## Save simulation list as serialized data file
      simlist.file <- sprintf('__METASIM_output__%s_%s.rds', sim.id, key.strip)
...
## 
treat.rules <- c('below.benchmark')  ## 'random'
treat.thresholds <- c(0.5, 0.25)  ## lower threshold-->higher correlation with poor past performance
    ...
    for (i in 1:length(treat.rules)) {
      ...
      for (ii in 1:length(treat.thresholds)) {
        ...
        for (j in 1:length(prior.sd.scenarios)) {
          ...
          ##--------------------------------------------
          key <- sprintf('g%s|h%s|i%s|ii%s|j%s', g,h,i,ii,j)
          cat(sprintf('\n%s\n',key))
